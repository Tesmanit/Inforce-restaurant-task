...
from restaurant.models import Menu, Restaurant, Dish, Vote
from employees.models import Employee
from datetime import date

class FullFlowTests(APITestCase):
    def setUp(self):
        self.employee_user = User.objects.create_user(username="employee", password="testpass")
        self.restaurant = Restaurant.objects.create(name="MyPlace", user=self.employee_user)
        self.employee = Employee.objects.create(user=self.employee_user, restaurant=self.restaurant)

        self.dish = Dish.objects.create(name="Burger", calories=300, price=14.99)
        self.menu = Menu.objects.create(name="Lunch", restaurant=self.restaurant, day=date.today())
